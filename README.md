...
```
.
├── docs/01_mac_m4_setup.md         # M4 + 16 GB 环境搭建 (AVD、内存分配、root、zram)
├── docs/02_experiment_protocol.md  # 实验流程、指标 <-> 论文符号对照、注意事项
...
│   ├── runner.py                       # 真机(模拟器)实验循环 -> results/*.jsonl
...
# 1. 创建可 root 的 arm64 AVD (google_apis, 非 Play 镜像), 客体内存 3 GB
scripts/setup_avd.sh 35 cf_api35 3072

# 2. 启动模拟器并等待开机 (高压实验可改为 2048)
scripts/start_emulator.sh cf_api35 3072
...
# 6. 单次实验 / 参数扫描
python3 run_experiment.py configs/emulator_base.json --policy landlord --eta 0.3 --T 40
